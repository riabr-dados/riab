...
df = pd.read_csv(
    RAW / f"ibge-deflator-ipca/{SNAP}/deflator_ipca_base2024.csv",
    encoding="utf-8-sig",
    sep=None,
    engine="python",
)
# normalise column names (BOM + quotes)
df.columns = [c.strip().strip('"').lstrip("\ufeff").lower() for c in df.columns]
...
df2 = pd.read_csv(
    RAW / f"ancine-preco-medio-ingresso/{SNAP}/preco_medio_ingresso_ancine.csv",
    encoding="latin1",
    sep=None,
    engine="python",
)
df2.columns = [c.strip().strip('"').lstrip("\ufeff").lower() for c in df2.columns]
